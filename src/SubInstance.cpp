...
[[nodiscard]] bool SubInstance::is_corrupted() const {
    idx_t j_counter = 0;
    for (auto &col : cols) {
...
std::vector<idx_t> SubInstance::add_columns(const Cols &new_cols) {
...
void SubInstance::update_sol_costs(const std::vector<idx_t> &local_sol, real_t sol_cost) {
...
NO_INLINE real_t SubInstance::price(const std::vector<real_t> &u_k) {
...
idx_t SubInstance::fix_columns(std::vector<idx_t> &local_idxs_to_fix, std::vector<real_t> &u_star) {
...
void SubInstance::replace_columns(const std::vector<idx_t> &glob_cols_idxs) {
...
real_t SubInstance::get_global_LB(const std::vector<real_t> &u_k) {
...
idx_t SubInstance::find_local_col_idx(idx_t gj) {
...
std::vector<idx_t> SubInstance::get_localized_solution(const std::vector<idx_t> &glob_sol) {
...
void SubInstance::_init_priced_cols(Priced_Columns &_priced_cols) {
...
real_t SubInstance::_price_active_cols(const std::vector<real_t> &u_k, Priced_Columns &_priced_cols) {
...
NO_INLINE void SubInstance::_select_C0_cols(Priced_Columns &_priced_cols, std::vector<idx_t> &global_col_idxs) {
...
NO_INLINE void SubInstance::_select_C1_cols(Priced_Columns &_priced_cols, MStar &_covering_times, std::vector<idx_t> &global_col_idxs) {
...
NO_INLINE void SubInstance::_select_C2_cols(Priced_Columns &_priced_cols, MStar &_covering_times, std::vector<idx_t> &global_col_idxs) {
